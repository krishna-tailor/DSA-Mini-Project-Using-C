...
void serve_Patient()
{
    ...
    // ========= Deleting The File Appointment If No Patient Is Remainging To Serve =========

    if (f == 0)
    {
        system("clear");
        printf("<<<<< No Patient Remaining To Serve <<<<<");
    ...
    add_doctor(temp[0].Doctor_choice); // Adding Doctor Again to File
}
